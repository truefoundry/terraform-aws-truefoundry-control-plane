...
| [aws_iam_role.aurora_secondary_monitoring](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role) | resource |
...
| [aws_iam_role_policy_attachment.aurora_secondary_monitoring](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/iam_role_policy_attachment) | resource |
...
| [aws_route.dr_to_primary](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/route) | resource |
| [aws_route.primary_to_dr](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/route) | resource |
...
| [aws_vpc_peering_connection.primary_to_dr](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/vpc_peering_connection) | resource |
...
| [aws_vpc_peering_connection_options.primary](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/vpc_peering_connection_options) | resource |
| [aws_vpc_peering_connection_options.secondary](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/vpc_peering_connection_options) | resource |
...
| [aws_iam_policy_document.aurora_secondary_monitoring_assume](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document) | data source |
...
| [aws_vpc.peering_primary](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/vpc) | data source |
| [aws_vpc.peering_secondary](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/vpc) | data source |
...
| <a name="input_truefoundry_aurora_alarm_treat_missing_data"></a> [truefoundry\_aurora\_alarm\_treat\_missing\_data](#input\_truefoundry\_aurora\_alarm\_treat\_missing\_data) | How the replication-lag alarm treats missing AuroraGlobalDBReplicationLag<br/>data points. AWS values: missing, breaching, notBreaching, ignore.<br/><br/>Defaults to "missing" (safer): a metric gap alone will not drive the alarm<br/>into ALARM. Set to "breaching" only if you want a disappearing metric —<br/>which can indicate the primary region is down — to count toward failover.<br/>Combine with a higher truefoundry\_aurora\_alarm\_evaluation\_periods to avoid<br/>reacting to transient gaps. | `string` | `"missing"` | no |
...
| <a name="input_truefoundry_aurora_enable_automated_failover"></a> [truefoundry\_aurora\_enable\_automated\_failover](#input\_truefoundry\_aurora\_enable\_automated\_failover) | Enable fully automated cross-region failover. When true, the replication-lag<br/>CloudWatch alarm triggers an EventBridge rule that invokes the failover<br/>Lambda, which promotes the DR cluster to writer WITHOUT human intervention.<br/><br/>Disabled by default because an automatic global promotion during transient<br/>replication lag or a metric gap can cause an unplanned failover while the<br/>primary region is still healthy. When false, the alarm and SNS topic are<br/>still created (so you get alerted), and the Lambda can still be invoked<br/>manually — but no automatic promotion happens. | `bool` | `false` | no |
| <a name="input_truefoundry_aurora_secondary_config"></a> [truefoundry\_aurora\_secondary\_config](#input\_truefoundry\_aurora\_secondary\_config) | Configuration for the secondary Aurora cluster in the DR region (aws.secondary provider).<br/>Required when truefoundry\_db\_enable\_global\_cluster = true. | <pre>object({<br/>    cluster_identifier            = string<br/>    vpc_id                        = string<br/>    subnet_ids                    = list(string)<br/>    instance_class                = optional(string, "db.r6g.large")<br/>    instance_count                = optional(number, 1)<br/>    ingress_cidr_blocks           = optional(list(string), [])<br/>    ingress_security_group_ids    = optional(list(string), [])<br/>    additional_security_group_ids = optional(list(string), [])<br/>    publicly_accessible           = optional(bool, false)<br/>    backup_retention_period       = optional(number, 1)<br/>    kms_key_id                    = optional(string, null)<br/>    enable_insights               = optional(bool, false)<br/>    enable_monitoring             = optional(bool, false)<br/>    monitoring_interval           = optional(number, 5)<br/>    monitoring_role_arn           = optional(string, "")<br/>    tags                          = optional(map(string), {})<br/>  })</pre> | `null` | no |
| <a name="input_truefoundry_aurora_vpc_peering_allow_remote_dns_resolution"></a> [truefoundry\_aurora\_vpc\_peering\_allow\_remote\_dns\_resolution](#input\_truefoundry\_aurora\_vpc\_peering\_allow\_remote\_dns\_resolution) | Enable cross-VPC private DNS resolution on the peering. With this on,<br/>RDS/Aurora endpoint hostnames resolve to private IPs across the peering,<br/>so clients in one VPC can reach the peer cluster via private addresses<br/>transparently. Recommended.<br/><br/>Only used when truefoundry\_aurora\_vpc\_peering\_enabled = true. | `bool` | `true` | no |
| <a name="input_truefoundry_aurora_vpc_peering_dr_route_table_ids"></a> [truefoundry\_aurora\_vpc\_peering\_dr\_route\_table\_ids](#input\_truefoundry\_aurora\_vpc\_peering\_dr\_route\_table\_ids) | Route table IDs in the DR VPC that should receive a route to the primary<br/>VPC CIDR via the peering connection.<br/><br/>Leave empty if you'll add the routes elsewhere. Only used when<br/>truefoundry\_aurora\_vpc\_peering\_enabled = true. | `list(string)` | `[]` | no |
| <a name="input_truefoundry_aurora_vpc_peering_enabled"></a> [truefoundry\_aurora\_vpc\_peering\_enabled](#input\_truefoundry\_aurora\_vpc\_peering\_enabled) | Create cross-region VPC peering between the primary VPC (var.vpc\_id) and<br/>the DR VPC (truefoundry\_aurora\_secondary\_config.vpc\_id). Only effective<br/>when truefoundry\_db\_enable\_global\_cluster = true AND<br/>truefoundry\_aurora\_secondary\_config is set.<br/><br/>Aurora Global Database replication itself does NOT need this — AWS<br/>replicates over its own backbone. Enable when resources in one VPC must<br/>reach the database endpoint in the other VPC privately (e.g. primary-VPC<br/>apps reading from the DR reader endpoint, cross-region operators,<br/>monitoring crossing regions).<br/><br/>Requires non-overlapping VPC CIDRs. Leave disabled if you already have<br/>connectivity via Transit Gateway, an existing peering, or VPN. | `bool` | `false` | no |
| <a name="input_truefoundry_aurora_vpc_peering_primary_route_table_ids"></a> [truefoundry\_aurora\_vpc\_peering\_primary\_route\_table\_ids](#input\_truefoundry\_aurora\_vpc\_peering\_primary\_route\_table\_ids) | Route table IDs in the primary VPC that should receive a route to the DR<br/>VPC CIDR via the peering connection. Typically the route tables<br/>associated with the subnets whose workloads need to reach the DR cluster<br/>(for example EKS node subnets, app subnets).<br/><br/>Leave empty if you'll add the routes elsewhere — the peering connection<br/>will still be created, but no routes will be added. Only used when<br/>truefoundry\_aurora\_vpc\_peering\_enabled = true. | `list(string)` | `[]` | no |
...
| <a name="input_truefoundry_db_enable_global_cluster"></a> [truefoundry\_db\_enable\_global\_cluster](#input\_truefoundry\_db\_enable\_global\_cluster) | Enable Aurora Global Database with a secondary cluster in a DR region. Ignored when truefoundry\_db\_engine\_mode = "rds". The secondary region's provider must be passed as aws.secondary. | `bool` | `false` | no |
...
| <a name="input_truefoundry_s3_attach_cors_policy"></a> [truefoundry\_s3\_attach\_cors\_policy](#input\_truefoundry\_s3\_attach\_cors\_policy) | Attach CORS policy for the S3 bucket. When false, Terraform will not manage CORS, allowing it to be configured externally. | `bool` | `true` | no |
...
| <a name="output_truefoundry_aurora_vpc_peering_id"></a> [truefoundry\_aurora\_vpc\_peering\_id](#output\_truefoundry\_aurora\_vpc\_peering\_id) | Cross-region VPC peering connection ID. Empty when peering is disabled. |
| <a name="output_truefoundry_aurora_vpc_peering_status"></a> [truefoundry\_aurora\_vpc\_peering\_status](#output\_truefoundry\_aurora\_vpc\_peering\_status) | Cross-region VPC peering accepter status (e.g. 'active'). Empty when peering is disabled. |

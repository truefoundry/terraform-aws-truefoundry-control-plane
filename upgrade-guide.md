...
| New variables (additive) | — | `truefoundry_db_engine_mode`, `truefoundry_aurora_*` (engine version, instance class/count, log exports, global cluster toggles, secondary config, alert email, alarm evaluation periods), `truefoundry_aurora_vpc_peering_enabled` + supporting route-table / DNS resolution inputs |
